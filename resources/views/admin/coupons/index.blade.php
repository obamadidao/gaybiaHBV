...
<div class="card-body">
            ...
            <div class="filter-section mb-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="filterForm" method="GET" action="{{ route('admin.coupons.index') }}">
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    <label class="form-label">Tìm kiếm mã</label>
                                    <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Nhập mã giảm giá...">
                                </div>
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-select" name="type">
                                        ...
                                        <option value="fixed" {{ request('type') == 'fixed' ? 'selected' : '' }}>Tiền mặt</option>
                                    </select>
                                </div>
                                <div class="col-md-3 mb-3">
...
<td>
                                ...
                                @if($coupon->start_date && $coupon->end_date)
                                    ...
                                    @php
                                        ...
                                        $isActive = $now->between(\Carbon\Carbon::parse($coupon->start_date), \Carbon\Carbon::parse($coupon->end_date));
                                    @endphp
                                    <br>
                                    @if($isActive)
                                ...
                                @elseif($coupon->start_date)
                                    Từ {{ \Carbon\Carbon::parse($coupon->start_date)->format('d/m/Y') }}
                                    @php
                                        $isActive = \Carbon\Carbon::now()->gte(\Carbon\Carbon::parse($coupon->start_date));
                                    @endphp
                                    <br>
                                    @if($isActive)
                                        <span class="badge bg-success">Còn hiệu lực</span>
                                ...
                                @elseif($coupon->end_date)
                                    Đến {{ \Carbon\Carbon::parse($coupon->end_date)->format('d/m/Y') }}
                                    @php
                                        $isActive = \Carbon\Carbon::now()->lte(\Carbon\Carbon::parse($coupon->end_date));
                                    @endphp
                                    <br>
                                    @if($isActive)
                                        <span class="badge bg-success">Còn hiệu lực</span>
