...
<td>
                                                    ...
                                                    <span class="badge {{ $order->payment_status === 'paid' ? 'bg-success' : 'bg-warning' }}" id="payment-status-{{ $order->id }}">
...
<td>
                                                    ...
                                                    <span class="badge bg-{{ $order->status_badge_class }}" id="order-status-{{ $order->id }}">
                                                        {{ $order->status_text }}
</span>
</td>
...
<script>
       ...
       function updateOrderRowAfterCancel(orderId, reason) {
           ...
           if (statusBadge) {
                ...
                statusBadge.className = `badge bg-${getProfileStatusBadgeClass('cancelled')}`;
                statusBadge.textContent = getProfileStatusTextInVietnamese('cancelled');
           }
            // Optionally, disable the "Chi tiết" link or add a tooltip
            
            // Thêm hiệu ứng cho row bị hủy
       ...
       function updateOrderDetailModal(data) {
           ...
           if (orderIdInModal != data.order_id) return;
            ...
            if (data.new_status) {
                // Cập nhật badge trạng thái đơn hàng trong modal bằng ID cụ thể hoặc text matching
                const statusBadges = modalContent.querySelectorAll('.badge');
                statusBadges.forEach(badge => {
                    if (badge.textContent.includes('Chờ xử lý') || 
                        ...
                        badge.textContent.includes('Đã giao') ||
                        badge.textContent.includes('Đã hủy') ||
                        badge.textContent.includes('Đã hoàn tiền')) {
                        
                        badge.className = `badge bg-${getProfileStatusBadgeClass(data.new_status)}`;
                        badge.textContent = getProfileStatusTextInVietnamese(data.new_status);
                    }
                });
                ...
                const cancelOrderBtn = document.getElementById('cancelOrderBtn');
                if (cancelOrderBtn) {
                    if (data.new_status === 'pending' || data.new_status === 'confirmed' || data.new_status === 'processing') {
                        cancelOrderBtn.style.display = 'inline-block';
                    } else {
                        cancelOrderBtn.style.display = 'none';
                    }
            ...
            if (data.payment_status !== undefined) {
                const paymentStatusBadges = modalContent.querySelectorAll('.badge');
                paymentStatusBadges.forEach(badge => {
                    if (badge.textContent.includes('Đã thanh toán') || 
                        badge.textContent.includes('Chưa thanh toán')) {
                        
                        badge.className = `badge ${data.payment_status === 'paid' ? 'bg-success' : 'bg-warning'}`;
                        badge.textContent = data.payment_status === 'paid' ? 'Đã thanh toán' : 'Chưa thanh toán';
                    }
                });
                
                // Cập nhật payment status trong bảng đơn hàng
                updateOrderPaymentStatus(data.order_id, data.payment_status);
            }
        }

        // Cập nhật payment status trong bảng đơn hàng
        function updateOrderPaymentStatus(orderId, paymentStatus) {
            const paymentStatusBadge = document.getElementById('payment-status-' + orderId);
            if (paymentStatusBadge) {
                paymentStatusBadge.className = `badge ${paymentStatus === 'paid' ? 'bg-success' : 'bg-warning'}`;
                paymentStatusBadge.textContent = paymentStatus === 'paid' ? 'Đã thanh toán' : 'Chưa thanh toán';
       ...
       function addCancellationInfo(modalContent, data) {
        ...
        function getProfileStatusTextInVietnamese(status) {
            const statusTexts = {
                'pending': 'Chờ xử lý',
       ...
       window.addEventListener('load', function() {
           ...
           window.updateProfileOrderDetail = updateOrderDetailModal;
            window.updateProfilePaymentStatus = updateOrderPaymentStatus;
       });
   </script>
@endpush