...
// Lấy danh sách danh mục cha để filter
        ...
        $parentCategories = Category::whereNull('parent_id')->active()->orderBy('sort_order')->orderBy('name')->get();
...
private function getParentCategoriesForSelect($excludeId = null)
{
        ...
        $query = Category::whereNull('parent_id')->active()->orderBy('sort_order')->orderBy('name');
...
/**
    * Lấy danh sách danh mục theo hierarchy (cha -> con)
    */
private function getCategoriesInHierarchy($query)
{
// Lấy tất cả danh mục từ query với relationships
$allCategories = $query->with('parent', 'children')->get();

// Sắp xếp theo hierarchy
$hierarchicalCategories = collect();

// Lấy danh mục cha trước (parent_id = null)
...
// Thêm danh mục con của parent này
...
// Thêm danh mục con cấp 2 (grandchildren)
$grandchildren = $allCategories->where('parent_id', $child->id)
                    ->sortBy('sort_order')
                    ->sortBy('name');
                    ->sortBy([
                        ['sort_order', 'asc'],
...
// Thêm các danh mục orphan (không có parent trong kết quả query)
$orphanCategories = $allCategories->whereNotNull('parent_id')
->filter(function($category) use ($allCategories) {
return !$allCategories->contains('id', $category->parent_id);
})
            ->sortBy('sort_order')
            ->sortBy('name');
            ->sortBy([
...
$hierarchicalCategories->push($orphan);
}
...
$paginatedItems = $hierarchicalCategories->slice($offset, $perPage)->values();
