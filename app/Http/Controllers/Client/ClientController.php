...
class ClientController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // Tạo category giả để sử dụng với view category.blade.php
        $category = (object) [
            'id' => null,
        ...
        $searchQuery = Product::with(['category', 'primaryImage', 'reviews'])
            ->where('is_active', 1)
        ...
        switch ($sortBy) {
            case 'price-asc':
                $searchQuery->orderBy('base_price', 'asc');
                break;
            case 'price-desc':
                $searchQuery->orderBy('base_price', 'desc');
                break;
            case 'name-asc':
            ...
            case 'name-desc':
                ...
                break;
            case 'relevance':
                // Sắp xếp theo độ liên quan (tên chứa từ khóa trước)
            ...
            case 'featured':
            default:
                // Ưu tiên sản phẩm nổi bật, sau đó sắp xếp theo mới nhất
                $searchQuery->orderBy('is_featured', 'desc')
                    ->orderBy('created_at', 'desc');
                break;
        }

        // Paginate kết quả và giữ query parameters
        $products = $searchQuery->paginate(12)->withQueryString();

        // Log tìm kiếm để thống kê
        Log::info('Product search performed', [
            'query' => $query,
        ...
        return view('client.category', compact('category', 'products', 'sortBy', 'query'));
    }
